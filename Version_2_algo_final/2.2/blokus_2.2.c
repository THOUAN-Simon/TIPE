                ...
                for(int r=0; r<j2.pieces[j].nb_rotations; r++){
                    for(int t=0; t<=detect_col_vide_g(rotation(&j2.pieces[j])[r]); t++){
                        ...
                        if(candidat.piece[candidat.taille-1][candidat.taille-1] == '&'){
                            ...
                            result.nb_j2++;
                            free_rotation(rotation(&j2.pieces[j]), &j2.pieces[j]);
                            free_translation_g(translation_g(rotation(&j2.pieces[j])[r]),rotation(&j2.pieces[j])[r]);
                        }
                    }
                    if(detect_col_vide_g(rotation(&j2.pieces[j])[r]) != 0){
                        for(int t=0; t<=detect_col_vide_b(rotation(&j2.pieces[j])[r]); t++){
                            ...
                            if(candidat.piece[candidat.taille-1][candidat.taille-1] == '&'){
                                ...
                                result.nb_j2++;
                                free_rotation(rotation(&j2.pieces[j]), &j2.pieces[j]);
                                free_translation_b(translation_b(rotation(&j2.pieces[j])[r]),rotation(&j2.pieces[j])[r]);
                            }
                        }
                    }
                }
                        ...
                        if(j1.pieces[i].joue == false){
                            for(int r=0; r<j1.pieces[i].nb_rotations; r++){
                                for(int t=0; t<=detect_col_vide_g(rotation(&j1.pieces[i])[r]); t++){
                                    ...
                                    printf("ligne : %d ; colonne : %d ; piece : %d ; rotation : %d ; translation : %d ; existe  : %d \n", l, c, i, r, t, ok_ajoute);
                                    if(ok_ajoute == 1){
                                ...
                                if(detect_col_vide_g(rotation(&j1.pieces[i])[r]) != 0){
                                    for(int t=0; t<=detect_col_vide_b(rotation(&j1.pieces[i])[r]); t++){
                                        ...
                                        if(ok_ajoute == 1){
                                            ...
                                            supprime(grille,temp.ligne,temp.colonne,&temp.piece);
                                            free_rotation(rotation(&j1.pieces[i]), &j1.pieces[i]);
                                            free_translation_b(translation_b(rotation(&j1.pieces[i])[r]),rotation(&j1.pieces[i])[r]);
                                        }
                                        else{
                                            supprime(grille,l,c,candidat);
                                        }
...
int main (){
    ...
    ajoute(grille,0,0,&j1.pieces[11],0);
    ajoute(grille,17,17,&j2.pieces[15],1);
    coups_poss test = joue_coup(grille,j1,j2,2);
    //printf("\n nombre de coups j1 : %d \n", test.nb_j1);
    //printf("nombre de coups j2 : %d \n", test.nb_j2);
    afficher(grille);
    return 0;
}