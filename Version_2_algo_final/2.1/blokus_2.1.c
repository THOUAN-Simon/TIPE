...
struct points_diagonaux{
    dot points[100];   //le quart du plateau ne peut clairement pas être composée uniquement de points diagonaux dans une partie.
    int nb;
};
...
bool verif (char** grille, int posl, int posc, p a_placer, int tour){
    ...
    else if(tour > 1 && grille[19][19] != '&'){
        return false;
    }
    else if(a_placer.piece[0][0] != ' '){
        if(haut(posl,posc).existe){
            if(grille[haut(posl,posc).ligne][haut(posl,posc).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(gauche(posl,posc).existe){
            if(grille[gauche(posl,posc).ligne][gauche(posl,posc).colonne] == a_placer.joueur){
                return false;
            }
        }
    }
    else if(a_placer.piece[0][a_placer.taille-1] != ' '){
        if(haut(posl,posc+a_placer.taille-1).existe){
            if(grille[haut(posl,posc+a_placer.taille-1).ligne][haut(posl,posc+a_placer.taille-1).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(droit(posl,posc+a_placer.taille-1).existe){
            if(grille[droit(posl,posc+a_placer.taille-1).ligne][droit(posl,posc+a_placer.taille-1).colonne] == a_placer.joueur){
                return false;
            }
        }
    }
    else if(a_placer.piece[a_placer.taille-1][0] != ' '){
        if(bas(posl+a_placer.taille-1,posc).existe){
            if(grille[bas(posl+a_placer.taille-1,posc).ligne][bas(posl+a_placer.taille-1,posc).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(gauche(posl+a_placer.taille-1,posc).existe){
            if(grille[gauche(posl+a_placer.taille-1,posc).ligne][gauche(posl+a_placer.taille-1,posc).colonne] == a_placer.joueur){
                return false;
            }
        }
    }
    else if(a_placer.piece[a_placer.taille-1][a_placer.taille-1] != ' '){
        if(bas(posl+a_placer.taille-1,posc+a_placer.taille-1).existe){
            if(grille[bas(posl+a_placer.taille-1,posc+a_placer.taille-1).ligne][bas(posl+a_placer.taille-1,posc+a_placer.taille-1).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(droit(posl+a_placer.taille-1,posc+a_placer.taille-1).existe){
            if(grille[droit(posl+a_placer.taille-1,posc+a_placer.taille-1).ligne][droit(posl+a_placer.taille-1,posc+a_placer.taille-1).colonne] == a_placer.joueur){
                return false;
            }
        }
    }
    for(int i=1; i<a_placer.taille-1; i++){
        int l_haut = posl;
        int c_haut = posc + i;
        int l_bas = posl+a_placer.taille-1;
        int c_bas = posc + i;
        int l_gauche = posl + i;
        int c_gauche = posc;
        int l_droite = posl + i;
        int c_droite = posl+a_placer.taille-1;
        if(haut(l_haut,c_bas).existe){
            if(grille[haut(l_haut,c_bas).ligne][haut(l_haut,c_bas).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(bas(l_bas,c_bas).existe){
            if(grille[bas(l_bas,c_bas).ligne][bas(l_bas,c_bas).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(droit(l_droite,c_droite).existe){
            if(grille[droit(l_droite,c_droite).ligne][droit(l_droite,c_droite).colonne] == a_placer.joueur){
                return false;
            }
        }
        else if(gauche(l_gauche,c_gauche).existe){
            if(grille[gauche(l_gauche,c_gauche).ligne][gauche(l_gauche,c_gauche).colonne] == a_placer.joueur){
                return false;
            }
        }
    }
            ...
            if(j1.pieces[i].joue == false){
                for(int r=0; r<j1.pieces[i].nb_rotations; r++){
                    for(int t=0; t<=detect_col_vide_g(rotation(&j1.pieces[i])[r]); t++){
                        p candidat = *translation(rotation(&j1.pieces[i])[r])[t];
                        if(candidat.piece[0][0] == '.'){
                            ...
                            result.nb_j1++;
                            ajoute(grille,temp.ligne,temp.colonne,&temp.piece,tour);
                            afficher(grille);
            ...
            if(j2.pieces[j].joue == false){
                for(int r=0; r<j2.pieces[j].nb_rotations; r++){
                    for(int t=0; t<=detect_col_vide_g(rotation(&j2.pieces[j])[r]); t++){
                        p candidat = *translation(rotation(&j2.pieces[j])[r])[t];
                        if(candidat.piece[candidat.taille-1][candidat.taille-1] == '&'){
                            coup temp;
                            temp.ligne = 19-candidat.taille+1;
                            temp.colonne = 19-candidat.taille+1;
                            temp.piece = candidat;
                ...
                for(int i=0; i<42; i++){
                    if(j1.pieces[i].joue == false){
                        for(int r=0; r<j1.pieces[i].nb_rotations; r++){
                            for(int t=0; t<=detect_col_vide_g(rotation(&j1.pieces[i])[r]); t++){
                                p* candidat = translation(rotation(&j1.pieces[i])[r])[t];
                                if(ajoute(grille,l,c,candidat,tour)){
                                    coup temp;
                                    temp.ligne = l;
                                    temp.colonne = c;
                                    temp.piece = *candidat;
...
int rubik_distance (p piece){
    ...
    else if(piece.id == 5 || piece.id == 19){
        return 4;
    }
    else if(piece.id == 0){
        return 2;
...
//est optimale
coup maximise_p_diag (p piece, char** grille, int tour){
...
coup ployvalente (p piece, char** grille, char type_ennemi, int tour){
...
//indique la progression de la partie : -1 si j2 gagne, 1 si j1 gagne, 0 si il y a match nul.

int partie_terminee(ensemble j1, ensemble j2){}


//Algorithme glouton avec heuristique

coup glouton(char** grille, ensemble j1, ensemble j2){}
...
int main (){
    ...
    coups_poss test = joue_coup(grille,j1,j2,0);
    printf("\n nombre de coups de j1 : %d", test.nb_j1);
    printf("\n nombre de coups de j2 : %d \n", test.nb_j2);
    afficher(grille);
    return 0;
}